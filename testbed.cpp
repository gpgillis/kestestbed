...
const std::string APPLICATION_NAME = "TestBed - A POle Utility";
const std::string COMPANY_NAME = "Kurzweil Educational Systems";

POLE::Storage* storage = 0;

std::string _debugRequest = "off";
...
void visitLeaf( int indent, POLE::Storage* storage, std::string fullname, std::string name, std::ostream &os )
{
	...
	os << name;
	if( ss )
		if( !ss->fail() )os << "  (" << ss->size() << ")";
	os << std::endl;
#ifdef KESI
	if (!ss->fail())
	{
		CPoleLeaf *pLeaf = new CPoleLeaf(ss, 0);
		if (pLeaf->HasValues())
		{
			STRVALMAP::iterator valIt;
			if (pLeaf->GetFirstValueIterator(valIt))
			{
				while (pLeaf->IsValidIterator(valIt))
				{
					std::string valName;
					valName = valIt->first;
					std::string value;
					bool bIntFound = true;
					long lVal = pLeaf->GetInteger(valName.c_str(), 0xABCD);
					if (lVal == 0xABCD)
					{
						bIntFound = false;
						value = pLeaf->GetString(valName.c_str(), "");
					}
					else
					{
...
void visit( int indent, POLE::Storage* storage, std::string path, std::ostream &os )
{
	if (!storage->exists(path))
	{
		std::cout << "The specified storage node does not exist." << std::endl;
		return;
	}
	if (storage->isDirectory(path))
	{
		...
		for( it = entries.begin(); it != entries.end(); ++it )
		{
			...
			if (storage->isDirectory(fullname))
				visit( indent+1, storage, fullname + "/", os );
			else
				visitLeaf(indent+1, storage, fullname, name, os);
		}
	}
	else
		visitLeaf(indent, storage, path, path, os);
}
...
void cmdOpen(std::list<std::string> &entries)
{
	...
	else
	{
        ...
        // Set debugging flags
        if (_debugRequest.compare("off") == 0)
            storage->setDebug(false, false);
        else if (_debugRequest.compare("file") == 0)
            storage->setDebug(true, true);
        else if (_debugRequest.compare("con") == 0)
            storage->setDebug(true, false);
...
void cmdDebug(std::list<std::string> &entries)
{
    if (entries.size() < 1)
    {
        std::cout << "You must enter a debug arg : con|file|off" << std::endl;
        return;
    }

    std::string arg = entries.front();
    if (arg.compare("off") == 0 || (arg.compare("file") == 0) || arg.compare("con") == 0)
        _debugRequest = arg;
    else
        std::cout << "An invalid argument has been supplied [" << arg << "].  Valid args are con|file|off" << std::endl;
}
...
void cmdAudit(std::list<std::string> &entries)
{
	if (storage)
	{
		std::string errMsg;
		if (storage->audit(errMsg) != 0)
			printf("%s\n", errMsg.c_str());
		else
			printf("Success\n");
	}
...
void cmdAuditMacBug(void)
{
    if (storage)
    {
        std::string errMsg;
        bool repaired = false;
        if (storage->auditForKESMacBug(false, &repaired))
        {
            printf("%s\n", errMsg.c_str());
        }
        else
...
void cmdVisit(std::list<std::string> &entries)
{
	...
	else
	{
		...
		if (entries.size() == 2)
		{
			std::string filePath = entries.back();
			entries.pop_back();
			fout.open(filePath.c_str());
			fp = &fout;
		}
		if (entries.size() > 0)
			visit(0, storage, entries.front(), *fp);
		else
			visit(0, storage, "/", *fp);
	}
}
...
void cmdHelp(std::list<std::string> &entries)
{
	...
	std::cout << "visit [ssPath] [outFile] - list contents of the structured storage, or of one leaf or node" << std::endl;
	...
	std::cout << "audit - verify that the open storage looks correct" << std::endl;
    std::cout << "auditMacBug - audit the open storage for the Mac bug" << std::endl;
    std::cout << "debug con|file|off sets debugging data function" << std::endl;
...
int main(int argc, char* argv[])
{
	...
	while (true)
	{
		...
		if (cmdWord.compare("open") == 0)
			...
			cmdStats(entries);
		else if (cmdWord.compare("audit") == 0)
			cmdAudit(entries);
        else if (cmdWord.compare("auditMacBug") == 0)
            cmdAuditMacBug();
        else if (cmdWord.compare("debug") == 0)
            cmdDebug(entries);
